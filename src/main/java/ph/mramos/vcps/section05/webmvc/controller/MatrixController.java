...
public class MatrixController {
	...
	public String test3(@PathVariable int id, @MatrixVariable Map<String, String> matrix3) { // Must have no name or pathVar.
		System.out.println("id: " + id);
		System.out.println("matrix3: " + matrix3);
		return "person";
	}

	@GetMapping("test4/{id}") // http://localhost:8080/vcp-spring/matrix/test4/1;firstName=max;lastName=ramos
	public String test4(@PathVariable int id, @MatrixVariable(pathVar = "id") Map<String, String> matrix4) { // Must be pathVar.
		System.out.println("id: " + id);
	...
	public String test5(@PathVariable int id, @MatrixVariable String firstName, @MatrixVariable String lastName) { // Must have a name.
		System.out.println("id: " + id);
	...
	@GetMapping("test6/{info1}/stat{info2}/{id}") // http://localhost:8080/vcp-spring/matrix/test6/test;firstName=max;lastName=ramos/stat;age=34;birthDate=1988-05-11/1;weight=103;height=180
	public String test6( // Note that pathVar (e.g. company, employee) can be used to differentiate same key (e.g. name). Example: company.name, employee.name
			...
			@MatrixVariable String firstName,
			@MatrixVariable String lastName,
			@MatrixVariable String age,
			@MatrixVariable String birthDate,
			@MatrixVariable String weight,
			@MatrixVariable String height) {
		System.out.println("id: " + id);
